...
OmniLite is a fork of Litecoin Core, with the Omni Protocol feature support from OmniLayer for Bitcoin added as a new layer of functionality on top. As such interacting with the API is done in the same manner (JSON-RPC) as Litecoin Core, simply with additional RPCs available for utilizing Omni Protocol features.

As all existing Litecoin Core functionality is inherent to OmniLite, the RPC port by default remains as `9332` as per Litecoin.  If you wish to run OmniLite in tandem with Litecoin Core (eg. via a separate datadir) you may utilize the `-rpcport<port>` option to nominate an alternative port number.
...
| `amountdesired`     | string  | required | the amount of litecoins desired                                                               |
...
| `amountdesired`     | string  | required | the new amount of litecoins desired                                                           |
...
| `toaddress`         | string  | required | the address of the seller                                                                    |
| `propertyid`        | number  | required | the identifier of the token to purchase                                                      |
| `amount`            | string  | required | the Litecoin amount to send                                                                   |
...
$ omnicore-cli "omni_sendissuancecrowdsale" \
    "3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo" 2 1 0 "Companies" "Litecoin Mining" \
...
$ omnicore-cli "omni_sendissuancefixed" \
    "3Ck2kEGLJtZw9ENj2tameMCtS3HB7uRar3" 2 1 0 "Companies" "Litecoin Mining" \
...
$ omnicore-cli "omni_sendissuancemanaged" \
    "3HsJvhr9qzgRe3ss97b1QHs38rmaLExLcH" 2 1 0 "Companies" "Litecoin Mining" "Quantum Miner" "" ""
```
...
Result:
{
  ...
  "litecoincoreversion" : "x.x.x",       // (string) Litecoin Core version
...
```js
{
  ...
  "fee" : "n.nnnnnnnn",            // (string) the transaction fee in litecoins
...
[                                  // (array of JSON objects)
  {
    ...
    "seller" : "address",              // (string) the Litecoin address of the seller
    "amountavailable" : "n.nnnnnnnn",  // (string) the number of tokens still listed for sale and currently available
    "litecoindesired" : "n.nnnnnnnn",   // (string) the number of litecoins desired in exchange
    ...
    "accepts": [                       // (array of JSON objects) a list of pending "accept" orders
      {
        "buyer" : "address",               // (string) the Litecoin address of the buyer
        ...
        "amounttopay" : "n.nnnnnnnn"       // (string) the amount in litecoins needed finalize the trade
      },
      ...
    ]
  },
...
```js
{
  ...
  "recipients": [                // (array of JSON objects) a list of recipients
    {
      "address" : "address",         // (string) the Litecoin address of the recipient
...
```bash
$ omnicore-cli "omni_createpayload_issuancefixed" 2 1 0 "Companies" "Litecoin Mining" "Quantum Miner" "" "" "1000000"
```
...
```bash
$ omnicore-cli "omni_createpayload_issuancecrowdsale" 2 1 0 "Companies" "Litecoin Mining" "Quantum Miner" "" "" 2 "100" 1483228800 30 2
```
...
```bash
$ omnicore-cli "omni_createpayload_issuancemanaged" 2 1 0 "Companies" "Litecoin Mining" "Quantum Miner" "" ""
```
